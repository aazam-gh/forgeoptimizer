...
type PersistedRun=Pick<Run,'id'|'repositoryUrl'|'sourceBranch'|'sourceCommitSha'|'scenarios'|'status'|'mode'|'approvalStatus'|'createdAt'|'updatedAt'|'failureReason'|'fallbackReason'|'trueForgeSessionId'|'trueForgeTurnId'|'events'|'candidates'|'usages'|'before'|'baseline'|'evaluations'|'validation'|'plan'|'branch'|'pullRequest'> & {policy?:unknown};
...
export function submitPersistedResults(id:string,results:{after:Run['after'];projection?:Run['projection'];validation?:Run['validation']}):Promise<PersistedRun>{return request<PersistedRun>(`/api/runs/${encodeURIComponent(id)}/results`,{method:'POST',body:JSON.stringify(results)});}
...
export function executeCandidate(candidateId:string):Promise<{runId:string;candidate:Run['candidates'][number];status:'queued'}>{return request(`/api/candidates/${encodeURIComponent(candidateId)}/execute`,{method:'POST'});}
export function getCandidateDiff(candidateId:string):Promise<{candidate:string;runId:string;diff:string}>{return request(`/api/candidates/${encodeURIComponent(candidateId)}/diff`);}
