...
When configured with a server-only `GITHUB_TOKEN`, the API exposes repository inspection via `POST /api/github/repository` and bounded source snapshots via `POST /api/github/source`. External branch, commit, and pull-request writes are available only through an approved persisted run under `/api/runs/:id/*`; unscoped write endpoints are rejected. The browser never receives the token, and the adapter never merges or pushes to the default branch.
