...
function openDatabase() {
  ...
  try { database.exec('ALTER TABLE optimization_runs ADD COLUMN requests_per_day REAL'); } catch { /* already migrated */ }
  ...
  try { database.exec('ALTER TABLE optimization_runs ADD COLUMN optimizer_usage_json TEXT'); } catch { /* already migrated */ }
  return database;
}
...
function runRecord(database, id) {
  ...
  const optimizerUsage = run.optimizer_usage_json ? JSON.parse(run.optimizer_usage_json) : undefined;
  return { id: run.id, repositoryUrl: run.repository_url, sourceBranch: run.source_branch ?? undefined, sourceCommitSha: run.source_commit_sha ?? undefined, requestsPerDay: run.requests_per_day ?? undefined, scenarios: JSON.parse(run.scenarios_json ?? '[]'), patchFiles: run.patch_files_json ? JSON.parse(run.patch_files_json) : undefined, validation: run.validation_json ? JSON.parse(run.validation_json) : undefined, status: run.status, mode: run.mode, approvalStatus: run.approval_status, policy: JSON.parse(run.policy_json), plan: run.plan_json ? JSON.parse(run.plan_json) : undefined, branch: run.branch_json ? JSON.parse(run.branch_json) : undefined, pullRequest: run.pull_request_json ? JSON.parse(run.pull_request_json) : undefined, baseline: run.baseline_json ? JSON.parse(run.baseline_json) : undefined, evaluations: run.evaluations_json ? JSON.parse(run.evaluations_json) : undefined, after: run.after_json ? JSON.parse(run.after_json) : undefined, projection: run.projection_json ? JSON.parse(run.projection_json) : undefined, optimizerUsage, optimizerCost: optimizerUsage?.cost, candidates: JSON.parse(run.candidates_json), usages: JSON.parse(run.usages_json), before: JSON.parse(run.before_json), createdAt: run.created_at, updatedAt: run.updated_at, failureReason: run.failure_reason ?? undefined, fallbackReason: run.fallback_reason ?? undefined, trueForgeSessionId: run.trueforge_session_id ?? undefined, trueForgeTurnId: run.trueforge_turn_id ?? undefined, events };
}
...
async function handleRequest(request, response, next, database, config = {}) {
  ...
  try {
    ...
    if (request.method === 'POST' && segments[3] === 'results') { const run = runRecord(database, id); if (!run) return json(response, 404, { error: 'Run not found' }); const body = await readBody(request); if (!body.after) return json(response, 400, { error: 'After metrics are required' }); database.prepare('UPDATE optimization_runs SET after_json = ?, projection_json = COALESCE(?, projection_json), validation_json = COALESCE(?, validation_json), patch_files_json = COALESCE(?, patch_files_json), optimizer_usage_json = COALESCE(?, optimizer_usage_json), updated_at = ? WHERE id = ?').run(JSON.stringify(body.after), body.projection !== undefined ? JSON.stringify(body.projection) : null, body.validation !== undefined ? JSON.stringify(body.validation) : null, body.patchFiles !== undefined ? JSON.stringify(body.patchFiles) : null, body.optimizerUsage !== undefined ? JSON.stringify(body.optimizerUsage) : null, new Date().toISOString(), id); return json(response, 200, runRecord(database, id)); }
    ...
    if (request.method === 'POST' && segments[3] === 'complete') return json(response, 200, updateRun(database, id, { status: 'completed' }));
    ...
    if (request.method === 'POST' && segments[3] === 'github-branch') { const run = runRecord(database, id); if (!run) return json(response, 404, { error: 'Run not found' }); if (run.approvalStatus !== 'approved') return json(response, 409, { error: 'Explicit approval is required before creating an optimization branch' }); const body = await readBody(request); const baseBranch = run.branch?.baseBranch ?? body.baseBranch; const branch = await createOptimizationBranch(run.repositoryUrl, baseBranch, body.branchName, run.branch?.baseCommitSha ?? run.sourceCommitSha); database.prepare('UPDATE optimization_runs SET source_branch = ?, source_commit_sha = ?, branch_json = ?, updated_at = ? WHERE id = ?').run(baseBranch, branch.baseCommitSha, JSON.stringify({ baseBranch, baseCommitSha: branch.baseCommitSha, optimizationBranch: branch.branchName }), new Date().toISOString(), id); return json(response, 201, runRecord(database, id)); }
