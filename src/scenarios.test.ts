...
import { defaultScenarios, executeScenario, executeScenarios, validateScenario, validateScenarios } from './scenarios';
...
describe('sandbox-backed optimization scenarios',()=>{
  ...
  it('executes every configured scenario and preserves result order',async()=>{const seen:string[]=[];const results=await executeScenarios({execute:async request=>{seen.push(request.scenario.id);return{scenarioId:request.scenario.id,status:'passed',quality:'MEASURED'};}},{repositoryUrl:'fixture://app',commitSha:'abc',scenarios:[...defaultScenarios]});expect(seen).toEqual(['fixture-tests','fixture-typecheck']);expect(results.every(result=>result.status==='passed')).toBe(true);});
});
