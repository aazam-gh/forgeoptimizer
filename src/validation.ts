...
export function approveValidationGate(snapshot:ValidationGateResult):ValidationGateResult{const checks:Record<string,ValidationState>={...snapshot.checks,review:'PASS'};const nonReview=Object.entries(checks).filter(([name])=>name!=='review').map(([,state])=>state);if(nonReview.includes('FAIL'))return{...snapshot,checks,state:'FAIL',canPublish:false,reasons:[...new Set([...snapshot.reasons,'Validation evidence failed'])]};if(nonReview.includes('NOT_VERIFIED'))return{...snapshot,checks,state:'NOT_VERIFIED',canPublish:false,reasons:[...new Set([...snapshot.reasons,'Validation evidence is incomplete'])]};return{state:'PASS',canPublish:true,checks,reasons:[]};}
