...
export interface Candidate { id:string; usageId:string; file:string; line:number; category:Category; title:string; finding:string; recommendation:string; savingsPercent:number; confidence:Confidence; risk:'LOW'|'MEDIUM'|'HIGH'; removesAi:boolean; changeType?:'code'|'model'|'prompt'|'dependency'|'ai_removal'; accepted?:boolean; dependsOn?:string[]; diff:string; frequencyPerDay?:number; testCoverage?:number; blastRadius?:number; complexity?:number; measurementQuality?:MetricQuality; }
export interface RunMetrics { calls:number; tokens:number; cost:number; latencyMs:number; quality:MetricQuality; }
export interface ValidationSnapshot { state:'PASS'|'FAIL'|'NOT_VERIFIED'; canPublish:boolean; checks:Record<string,'PASS'|'FAIL'|'NOT_VERIFIED'>; reasons:string[]; }
...
export interface Run { id:string; repositoryUrl:string; sourceBranch?:string; sourceCommitSha?:string; scenarios?:OptimizationScenario[]; status:RunStatus; approvalStatus:'pending'|'approved'; createdAt:string; updatedAt?:string; stage?:string; mode?:RunMode; failureReason?:string; fallbackReason?:string; activeCandidateId?:string; trueForgeSessionId?:string; trueForgeTurnId?:string; activeSandboxId?:string; retryCount?:number; startedAt?:string; completedAt?:string; usages:AiUsage[]; candidates:Candidate[]; before:RunMetrics; after?:RunMetrics; baseline?:BaselineProfile; evaluations?:EvaluationResult[]; validation?:ValidationSnapshot; plan?:OptimizationPlan; projection?:CostProjection; branch?:GitBranchRecord; pullRequest?:PullRequestRecord; events:AgentEvent[]; }
...
export interface ScenarioExecutionResult { scenarioId:string; status:'passed'|'failed'|'timed_out'|'not_verified'; exitStatus?:number; stdout?:string; stderr?:string; durationMs?:number; quality:'MEASURED'|'NOT_VERIFIED'; baselineFailure?:boolean; }
...
export interface OptimizationPolicy { maxBehavioralRisk:'low'|'medium'|'high'; minimumExpectedSavingsPercent:number; allowModelChanges:boolean; allowPromptChanges:boolean; allowDependencyChanges:boolean; allowAiRemoval:boolean; maxFilesPerPatch:number; requireAllTests:boolean; requireReviewAgent:boolean; }
