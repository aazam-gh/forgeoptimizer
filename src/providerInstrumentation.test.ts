...
import { instrumentAnthropic, instrumentGemini, instrumentLangChainJS, instrumentLangChainPython, instrumentLlamaIndex, instrumentOpenAI, instrumentVercelAI } from './providerInstrumentation';
...
describe('normalized provider instrumentation',()=>{
  it('normalizes OpenAI usage and finish metadata',async()=>{const result=await instrumentOpenAI(input,async()=>({model:'gpt-4.1',usage:{prompt_tokens:12,completion_tokens:4},finish_reason:'stop'}));expect(result.invocation.inputTokens).toBe(12);expect(result.invocation.outputTokens).toBe(4);expect(result.invocation.cost).toBeCloseTo(0.000056);expect(result.invocation.metadata).toMatchObject({provider:'OpenAI',finishReason:'stop'});});
