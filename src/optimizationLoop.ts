...
export type CandidateAttempt = {
  candidateId: string;
  status: "passed" | "reverted" | "skipped";
  detail: string;
  durationMs: number;
};
...
export async function runOptimizationLoop(
  ...
  for (const step of nextSteps) {
    ...
    if (!candidate) {
      ...
      attempts.push({
        ...
        status: "skipped",
        detail: "Candidate is missing from the run snapshot",
        durationMs: 0,
      });
      continue;
    }
    const blockedDependency = step.dependsOn.find(
      (dependencyId) =>
        nextSteps.find((dependency) => dependency.id === dependencyId)
          ?.status !== "passed",
    );
    if (blockedDependency) {
      step.status = "skipped";
      const dependency = nextSteps.find(
        (item) => item.id === blockedDependency,
      );
      const detail = `Dependency ${dependency?.candidateId ?? blockedDependency} did not pass`;
      attempts.push({
    ...
    try {
      ...
      if (result.passed) {
        ...
        attempts.push({
          ...
          detail: overRuntimeBudget
            ? "Candidate exceeded the runtime budget and was rejected"
            : result.detail,
          durationMs,
        });
        if (!passed) accepted.pop();
      } else {
  ...
  return {
    ...
    remainingCandidateIds: nextSteps
