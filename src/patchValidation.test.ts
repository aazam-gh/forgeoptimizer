...
describe("candidate patch validation", () => {
  it("extracts changed files and accepts a normal unified diff", () => {
    ...
    ).toEqual({ valid: true, changedFiles: ["src/app.ts"], errors: [] });
  });
  ...
  it("rejects traversal and binary patches before sandbox application", () => {
    ...
    expect(result.errors).toEqual(
      expect.arrayContaining([
        ...
        "unsafe changed file path: ../secrets.env",
      ]),
    );
  });
  it("rejects empty patches", () => {
    expect(validateCandidatePatch("")).toEqual({
      valid: false,
      changedFiles: [],
      errors: ["patch must not be empty"],
    });
  });
  it("accepts deletion-only patches and identifies the removed file", () => {
    expect(
      validateCandidatePatch(
        "--- a/src/legacy.ts\n+++ /dev/null\n@@\n-old code",
      ),
    ).toMatchObject({ valid: true, changedFiles: ["src/legacy.ts"] });
  });
});
