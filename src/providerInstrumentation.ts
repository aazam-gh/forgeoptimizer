...
> {
  const input =
    ...
    numberAt(response, "usage", "inputTokens");
  const output =
    numberAt(response, "usage", "completion_tokens") ??
    numberAt(response, "usage", "output_tokens") ??
    numberAt(response, "usage", "outputTokens");
  const finishReason =
  ...
  return {
    ...
    metadata: {
      provider,
      finishReason,
      cacheHit: numberAt(response, "usage", "cache_read_tokens") !== undefined,
    },
  };
}
export async function instrumentProviderCall<T extends ProviderResponse>(
  input: ProviderCallInput,
  operation: () => Promise<T>,
): Promise<{ value: T; invocation: AIInvocation }> {
  const result = await instrumentInvocation(
    {
...
export const instrumentOpenAI = (
  input: Omit<ProviderCallInput, "provider">,
  operation: () => Promise<ProviderResponse>,
) => instrumentProviderCall({ ...input, provider: "OpenAI" }, operation);
export const instrumentAnthropic = (
  input: Omit<ProviderCallInput, "provider">,
  operation: () => Promise<ProviderResponse>,
) => instrumentProviderCall({ ...input, provider: "Anthropic" }, operation);
export const instrumentGemini = (
  input: Omit<ProviderCallInput, "provider">,
  operation: () => Promise<ProviderResponse>,
) => instrumentProviderCall({ ...input, provider: "Google" }, operation);
export const instrumentVercelAI = (
  input: Omit<ProviderCallInput, "provider">,
  operation: () => Promise<ProviderResponse>,
) => instrumentProviderCall({ ...input, provider: "Vercel AI SDK" }, operation);
export const instrumentLangChainJS = (
  input: Omit<ProviderCallInput, "provider">,
  operation: () => Promise<ProviderResponse>,
) => instrumentProviderCall({ ...input, provider: "LangChain JS" }, operation);
export const instrumentLangChainPython = (
  input: Omit<ProviderCallInput, "provider">,
  operation: () => Promise<ProviderResponse>,
) =>
  instrumentProviderCall({ ...input, provider: "LangChain Python" }, operation);
export const instrumentLlamaIndex = (
  input: Omit<ProviderCallInput, "provider">,
  operation: () => Promise<ProviderResponse>,
) => instrumentProviderCall({ ...input, provider: "LlamaIndex" }, operation);
