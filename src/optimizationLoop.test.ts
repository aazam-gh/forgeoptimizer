...
const plan:OptimizationPlan={id:'plan-1',runId:'run-1',createdAt:'2026-01-01T00:00:00.000Z',steps:[{id:'step-a',candidateId:'a',title:'a',dependsOn:[],score:1,status:'queued'},{id:'step-b',candidateId:'b',title:'b',dependsOn:['step-a'],score:.5,status:'queued'}],expectedSavingsPercent:20,risk:'LOW',valid:true,validationErrors:[]};

describe('bounded autonomous optimization loop',()=>{
  it('keeps successful candidates and reverts a failed candidate',async()=>{
    const result=await runOptimizationLoop(plan,[candidate('a'),candidate('b')],async current=>({passed:current.id==='a',detail:current.id==='a'?'tests passed':'behavioral regression'}));
    expect(result.acceptedCandidateIds).toEqual(['a']);
    expect(result.plan.steps.map(step=>step.status)).toEqual(['passed','reverted']);
  });

  it('stops before exceeding candidate budget',async()=>{
    const result=await runOptimizationLoop(plan,[candidate('a'),candidate('b')],async()=>({passed:true,detail:'passed'}),{maxAgentCost:5,maxTrueForgeIterations:20,maxParallelSubAgents:4,maxCandidates:1,maxRuntimeMs:900000,maxSandboxExecutions:20});
    expect(result.acceptedCandidateIds).toEqual(['a']);
