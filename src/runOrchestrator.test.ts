...
const run: Run = {
  id: "run-1",
  repositoryUrl: "fixture://app",
  status: "created",
  approvalStatus: "pending",
  createdAt: "2026-01-01T00:00:00.000Z",
  usages: [],
  candidates: [],
  before: { calls: 0, tokens: 0, cost: 0, latencyMs: 0, quality: "MEASURED" },
  events: [],
};
...
describe("end-to-end optimization orchestration", () => {
  it("runs baseline, applies candidates one at a time, and reverts a failing candidate", async () => {
      ...
      [candidate("a"), candidate("b")],
      {
        ...
        sandbox: {
          execute: async (request) => ({
            ...
            status: request.commitSha === "c".repeat(40) ? "failed" : "passed",
            quality: "MEASURED",
          }),
        },
        applyCandidate: async (current) => {
          applied.push(current.id);
          return {
            commitSha: current.id === "a" ? "b".repeat(40) : "c".repeat(40),
          };
        },
        revertCandidate: async (current) => {
          reverted.push(current.id);
        },
    ...
    expect(result.workflow.commits.map((commit) => commit.status)).toEqual([
      "applied",
      "reverted",
    ]);
    expect(result.run.status).toBe("fallback");
    expect(result.validation.reasons).toContain(
      "Candidate scenario failed or timed out",
    );
    expect(calls).toBe(2);
  });
  ...
  it("gates model-change candidates on measured benchmark evidence", async () => {
    ...
    const result = await executeOptimizationRun(run, [modelCandidate], {
      ...
      revertCandidate: async () => {},
      benchmarkCandidate: async () => false,
    });
    expect(applied).toEqual([]);
    expect(result.workflow.commits).toEqual([]);
    expect(result.validation.reasons).toEqual(
      expect.arrayContaining([
        "Behavioral evaluation evidence is incomplete or failing",
      ]),
    );
  });

  it("uses the latest candidate scenario for the final validation gate", async () => {
    const result = await executeOptimizationRun(run, [candidate("bad")], {
      scenario,
      baselineCommitSha: "a".repeat(40),
      sandbox: {
        execute: async (request) => ({
          scenarioId: request.scenario.id,
          status: request.commitSha === "b".repeat(40) ? "failed" : "passed",
          quality: "MEASURED",
        }),
      },
      applyCandidate: async () => ({ commitSha: "b".repeat(40) }),
      revertCandidate: async () => {},
      evaluateCandidate: async () => [
        {
          caseId: "case",
  ...
  it("runs all configured scenarios for baseline and each candidate", async () => {
    const second = { ...scenario, id: "typecheck", name: "typecheck" };
    const result = await executeOptimizationRun(run, [candidate("a")], {
  ...
  it("applies the run policy when building the plan", async () => {
    const model = {
      ...candidate("model"),
      category: "Cheaper model" as const,
      changeType: "model" as const,
    };
    const result = await executeOptimizationRun(
      {
    ...
    expect(result.workflow.commits).toHaveLength(0);
  });
});
